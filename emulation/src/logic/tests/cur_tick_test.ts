...
Deno.test("CurrentTick - Deposit", () => {
    ...
    const curTick = new CurrentTick(factory.make(100), reserve, new Inventory());
...
Deno.test("CurrentTick - Increment", () => {
    ...
    // Now increment
    const initialIdx = curTick.index().toAbsolute();
    curTick.increment("inventory -> reserve");
    
    assertEquals(curTick.index().toAbsolute(), initialIdx + 1);
...
Deno.test("Full Simulation - Quote AMM with Recovery", () => {
    ...
    quoteReserve.init(1000, invertedFactory.make(1100), invertedFactory.make(1001));
    const quoteCurTick = new CurrentTick(invertedFactory.make(1000), quoteReserve, quoteInventory);
    ...
    assertEquals(quoteCurTick.index().toAbsolute(), 1000);
    ...
    let swapCount = 0;
    while (quoteCurTick.index().toAbsolute() < 1005 && swapCount < 20) {
        swapCount++;
        
        // Add fees for this trade
        quoteCurTick.addInventoryFees(0.5 * quoteCurTick.index().getPrice());
    ...
    assertEquals(quoteCurTick.index().toAbsolute() > 1000, true);
    ...
    const midPosition = quoteCurTick.index().toAbsolute();
    ...
    // Final verification
    const finalPosition = quoteCurTick.index().toAbsolute();
    ...
    console.log(`  Final inventory respectiveReserve: ${finalRespectiveReserve}`);
...
Deno.test("Perfect Reversal - No Fees", () => {
    ...
    // Capture initial state
    const initialPosition = quoteCurTick.index().toAbsolute();
    const initialReserveQty = quoteReserve.qty;
    
    console.log(`\nInitial state: tick ${initialPosition}, reserve ${initialReserveQty.toFixed(2)}`);
    ...
    while (swapQtyIn > 0) {
        console.log(`  [Phase 1] Tick ${quoteCurTick.index().toAbsolute()}: swapping ${swapQtyIn.toFixed(4)} inventory to reserve`);
        ...
        if (swapResult.tickExhausted && swapQtyIn > 0) {
            console.log(`  Tick exhausted at ${quoteCurTick.index().toAbsolute()}, moving to next...`);
            quoteCurTick.decrement("inventory -> reserve");
        }
    }
    
    const midPosition = quoteCurTick.index().toAbsolute();
    console.log(`Phase 1 complete: moved from ${initialPosition} to ${midPosition}`);
    console.log(`  Gave ${totalInventoryGiven.toFixed(2)} inventory, received ${totalReserveReceived.toFixed(2)} reserve`);
    console.log(JSON.stringify(quoteCurTick, undefined, 2));
    
    // Verify we moved ticks
    assertEquals(midPosition > initialPosition, true, "Should have moved to higher tick");
    ...
    console.log(`\nPhase 2: Swapping ${totalReserveReceived.toFixed(2)} reserve back for inventory...`);
    
    let totalInventoryReceived = 0;
    swapQtyIn = totalReserveReceived;
    
    while (swapQtyIn > 1e-9) {
        console.log(`  [Phase 2] Tick ${quoteCurTick.index().toAbsolute()}: swapping ${swapQtyIn.toFixed(4)} reserve`);
        ...
        console.log(`    Result: qtyOut=${swapResult.qtyOut.toFixed(4)}, reminderIn=${swapResult.reminderIn.toFixed(4)}, tickExhausted=${swapResult.tickExhausted}`);
        ...
        if (swapResult.tickExhausted && swapQtyIn > 1e-9) {
            ...
            if (swapResult.qtyOut === 0) {
                console.log(JSON.stringify(quoteCurTick, undefined, 2));
                console.log(`  No more inventory available, ending Phase 2 with ${swapQtyIn.toFixed(4)} reserve remaining`);
                break;
            }
            console.log(`  Tick exhausted at ${quoteCurTick.index().toAbsolute()}, moving back...`);
            quoteCurTick.increment("reserve -> inventory");
        }
    }
    ...
    console.log(`Phase 2 complete: moved from ${midPosition} back to ${finalPosition}`);
    console.log(`  Gave ${totalReserveReceived.toFixed(2)} reserve, received ${totalInventoryReceived.toFixed(2)} inventory`);
    console.log(`\nFinal state:`);
    console.log(`  Position: ${initialPosition} -> ${finalPosition}`);
    console.log(`  Reserve: ${initialReserveQty.toFixed(2)} -> ${finalReserveQty.toFixed(2)}`);
    console.log(`  Inventory empty: ${quoteInventory.isEmpty()}`);
    console.log(`  Inventory respectiveReserve: ${quoteInventory.getRespectiveReserve().toFixed(4)}`);
    
    // Perfect reversal verification
    assertEquals(finalPosition, initialPosition, "Should return to initial tick");
    assertAlmostEquals(totalInventoryReceived, totalInventoryGiven, 0.1, "Should get back same inventory");
    assertAlmostEquals(finalReserveQty, initialReserveQty, 0.1, "Reserve should be restored");
    assertEquals(quoteInventory.isEmpty(), true, "Inventory should be empty");
    assertAlmostEquals(quoteInventory.getRespectiveReserve(), 0, 0.1, "IL should be zero");
});
