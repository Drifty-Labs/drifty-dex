...
Deno.test("Pool - Swap Base -> Quote (Price Down)", () => {
    ...
    // Initial ticks
    const initialBaseTick = (pool as any).stableAMM.base
        .curTick()
        .index()
        .toAbsolute();
    const initialQuoteTick = (pool as any).stableAMM.quote
        .curTick()
        .index()
        .toAbsolute();
    ...
    const finalBaseTick = (pool as any).stableAMM.base
        .curTick()
        .index()
        .toAbsolute();
    ...
    console.log(
        `Base -> Quote: Moved from ${initialBaseTick} to ${finalBaseTick}`
    );
    assertEquals(
        finalBaseTick < initialBaseTick,
        true,
        "Price should decrease (move left)"
    );
});
...
Deno.test("Pool - Swap Quote -> Base (Price Up)", () => {
    ...
    console.log(
        `Quote -> Base: Moved from ${initialBaseTick} to ${finalBaseTick}`
    );
    assertEquals(
        finalBaseTick > initialBaseTick,
        true,
        "Price should increase (move right)"
    );
});
...
Deno.test("Pool - Full Simulation (Round Trip)", () => {
    ...
    while (
        (pool as any).stableAMM.base.curTick().index().toAbsolute() >
            targetTick &&
        swaps < 100
    ) {
        swaps++;
    ...
    const finalTick = (pool as any).stableAMM.base
        .curTick()
        .index()
        .toAbsolute();
