...
Deno.test("AMM - Full Simulation (Buy then Sell)", () => {
    ...
    while (amm.curTick().index().toAbsolute() > targetTickIdx && swaps < 100) {
        swaps++;
        const curPrice = amm.curTick().index().getPrice();
    ...
    assertEquals(midTickIdx < startTickIdx, true, "Should have moved price down");
    ...
    while (remainingReserveToSell > 1e-15 && swaps < 100 && amm.curTick().index().toAbsolute() <= startTickIdx) {
        swaps++;
    ...
    assertEquals(finalTickIdx >= startTickIdx, true, "Should have returned to start tick");
    assertEquals(finalTickIdx <= startTickIdx + 1, true, "Should not have overshot significantly");
    ...
    const actualReserve = (amm as any).reserve.qty + (amm.curTick() as any).currentReserve;
    assertAlmostEquals(actualReserve, 1000, 0.001, "Reserve should be restored");
});
