...
Deno.test("Reserve Manager - Lifecycle (Normal)", () => {
    ...
    // 2. Double Init Check
    assertThrows(() => reserve.init(100, left, right), Error, "already initted");
    ...
    assertThrows(() => reserve.takeBest(tick100), Error, "Reserve invariant violated");
...
Deno.test("Inventory Manager - Lifecycle & Flows", () => {
    ...
    assertThrows(() => inventory.putBest({ idx: tick102, inventory: 100 }), Error, "Inventory invariant violated");
    ...
    assertThrows(() => inventory.putWorstNewRange({ idx: tick104, inventory: 50 }), Error, "Inventory invariant violated");
});
...
Deno.test("Symmetry - Base vs Quote", () => {
    ...
    quoteReserve.init(1000, invertedFactory.make(1100), invertedFactory.make(1010));
...
Deno.test("Symmetry - Realistic Flow (Base -> Quote -> Base)", () => {
    ...
    for (let p = 1005; p >= 1001; p--) {
        ...
        quoteReserve.put(qInvTick!.inventory);
    }
